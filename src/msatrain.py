...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    for epoch in range(1, hyp_params.num_epochs + 1):
        ...
        if best_acc < acc:
            best_acc = acc
            # 保存最佳模型
            if hasattr(hyp_params, 'model_save_path') and hyp_params.model_save_path:
                save_dict = {
                ...
                torch.save(save_dict, hyp_params.model_save_path)
                print(f"最佳模型已保存到: {hyp_params.model_save_path}")
